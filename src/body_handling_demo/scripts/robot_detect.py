...
class Robot_Action:
    ...
    def __init__(self):
        ...
        # 力控夹爪控制
        self.right_gripper_pub = rospy.Publisher("/r_arm/rm_driver/Gripper_Pick", Gripper_Pick, queue_size=10)
        self.left_gripper_pub = rospy.Publisher("/l_arm/rm_driver/Gripper_Pick", Gripper_Pick, queue_size=10)
        # 开合夹爪控制
    ...
    def _get_rosparam(self):
        ...
        self.manual_mode_time = rospy.get_param("~manual_mode_time", 600)
        # 自主充电电量范围设置
        self.charge_range = rospy.get_param("~change_range", [30, 90])
        self.charge_pose = rospy.get_param("~charge_pose", "120")
        # 舵机角度设置
        self.servo_motor = rospy.get_param("~servo_motor", [350, 500])
        # self.servo_motor = [int(item) for item in rospy.get_param("~servo_motor", [350, 500])]

        rospy.loginfo(f"self.servo_motor=[{type(self.servo_motor)}, {self.servo_motor[0]}]")
    ...
    def _woosh_status(self, msg: RobotStatus):
        ...
        rospy.loginfo(f"msg.task_state={msg.task_state}")
    ...
    def _get_servo_angle(self, msg: ServoAngle):
        data = ServoMove()
        time.sleep(0.5)
        if msg.angle_1 > self.servo_motor[0] - 20 and msg.angle_1 < self.servo_motor[0] + 20:
            pass
        else:
            data.servo_id = 1
        ...
        if msg.angle_2 > self.servo_motor[1] - 20 and msg.angle_2 < self.servo_motor[1] + 20:
            pass
        else:
            data.servo_id = 2
    ...
    def _pick_up_the_box(self, detect_result):
        if detect_result.header.frame_id != '':
            ...
            # 规划到物体
            pose_vector = self._ar_tf_result(detect_result.pose, [0.142, -0.02, 0.083])
            self._movejp_plan(pose_vector[0]["orientation"], pose_vector[0]["position"], 1)
            self._movejp_plan(pose_vector[1]["orientation"], pose_vector[1]["position"], 2)
            # 等待双臂规划目标物完成
            self._wait_arm_planned(3)
    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            if not self._wait_manual_event.wait(timeout=self.manual_mode_time):   # 等5分钟
                rospy.logerr("Did not switch to the manual mode on time!")
                break
            else:
                rospy.loginfo("Please quickly refine the presentation scene to perform the next action!")
            timer.shutdown()
            ...
            if self._auto_electricize:
                ...
                if not self._wait_auto_event.wait(timeout=self.auto_mode_time):   # 等24小时
                    rospy.logerr("Charging time execeeds 24 hours, please check the line problem!")
                    break
                else:
                    rospy.loginfo("Automatic charding was performed succeessfully!")
                timer.shutdown()
            else:
                # 创建事件戳，判断机器人是否处于自动模式状态
                self._wait_auto_event.clear()
                timer = rospy.Timer(rospy.Duration(0.5), self._monitor_auto_status, oneshot=False)
                if not self._wait_auto_event.wait(timeout=self.auto_mode_time):   # 等24小时
                    rospy.logerr("Did not change the robot to manual mode for 24 hours!")
                else:
                    rospy.loginfo("Automatic charding was performed succeessfully!")
                timer.shutdown()
            ...
            # 双臂至识别目标物位姿
            self._movej_plan([-111.321, -7.430, -90.332, 45.072, -98.473, -25.041], 1)
            self._movej_plan([111.321, 7.430, 90.332, -45.072, 98.473, 25.041], 2)
            # 等待双臂规划完成
            self._wait_arm_planned(3)
            ...
            for _ in range(4):
                ...
                rospy.logwarn(f"{ '*' * 10 } { frame_id } { '*' * 10 }")
                if self._set_camera_status(True, frame_id):
                    detect_result = self._get_material_detect_result(self.label_name["1"], frame_id, 20.0)
                    q_l, v_l = self._analysis_materiel_detect_result(detect_result.pose)
                    if self._set_camera_status(False, frame_id):
                        # 切换工具坐标系至tip
                        coor_name = ChangeTool_Name()
                        coor_name.toolname = self.left_tool1
                        for _ in range(3):
                            self.set_left_tool_frame.publish(coor_name)
                            rospy.sleep(0.1)
                    else:
                    ...
                    v[2] += 0.03
                    v[0] -= 0.03
                    self._movejp_plan(q_l, v, 1)
                    self._wait_arm_planned(1)
                    self._movejp_plan(q_l, v_l, 1)
                    self._wait_arm_planned(1)
                    self._grip_materiel("close", 1)
                    rospy.sleep(3)
                    self._movejp_plan(q_l, v, 1)
                    self._wait_arm_planned(1)
                    # 过渡姿态
                    self._movej_plan([-120.000, -10.000, -75.000, 45.000, -110.000, -20.000], 1)
                    self._wait_arm_planned(1)
                    # 放置姿态
                    self._movej_plan([-121.120, -76.013, -36.879, 39.977, -75.353, -14.467], 1)
                    self._wait_arm_planned(1)
                ...
                if self._set_camera_status(True, frame_id):
                    detect_result = self._get_material_detect_result(self.label_name["1"], frame_id, 20.0)
                    q_r, v_r = self._analysis_materiel_detect_result(detect_result.pose)
                    if self._set_camera_status(False, frame_id):
                        # 切换工具坐标系至tip
                        coor_name = ChangeTool_Name()
                        coor_name.toolname = self.right_tool1
                        for _ in range(3):
                            self.set_right_tool_frame.publish(coor_name)
                            rospy.sleep(0.1)
                    else:
                    ...
                    v[0] += 0.03
                    v[2] += 0.03
                    self._movejp_plan(q_r, v, 2)
                    self._wait_arm_planned(2)
                    self._movejp_plan(q_r, v_r, 2)
                    self._wait_arm_planned(2)
                    self._grip_materiel("close", 2)
                    rospy.sleep(3)
                    self._movejp_plan(q_r, v, 2)
                    self._wait_arm_planned(2)

                    # 过渡姿态
                    self._movej_plan([120.000, 10.000, 75.000, -45.000, 110.000, 20.000], 2)
                    self._wait_arm_planned(2)
                    # 放置姿态
                    self._movej_plan([121.120, 76.013, 36.879, -39.977, 75.353, 14.467], 2)
                    self._wait_arm_planned(2)
            ...
            # 将机械臂伸进
            self._movej_plan([-75.226, -65.018, -90.613, 15.552, 66.080, -53.004], 1)
            self._movej_plan([75.226, 65.018, 90.613, -15.552, -66.080, 53.004], 2)
            # 等待双臂规划完成
            self._wait_arm_planned(3)
            ...
            if self._set_camera_status(True, frame_id):
                ...
                if self._set_camera_status(False, frame_id):
                    ...
                    rospy.logerr(f"The camera closed failed")
                    break
            else:
                break
            ...
            # 缩机械臂
            self._movej_plan([-78.478, -73.163, -125.281, 16.653, 107.488, -42.015], 1)
            self._movej_plan([74.952, 72.032, 127.261, -15.250, -108.318, 41.667], 2)
            # 等待双臂规划完成
            self._wait_arm_planned(3)

            rospy.loginfo(">>> Round %d of material capture is completed ...", 1)
    ...
    def _set_camera_status(self, run: bool, camera_name: str):
        ...
        if camera_result.result:
            rospy.loginfo("%s", camera_result.message)
            return True
        else:
            rospy.logerr("%s", camera_result.message)
            return False
    ...
    # ros时间戳回调汉书
    def _ros_timestemp_listen(self, event):
    ...
    def _navigation_wait(self, carry, timeout=120):
        ...
        # 创建时间戳
        timer = rospy.Timer(rospy.Duration(0.5), self._ros_timestemp_listen, oneshot=False)
        if carry:
            timeout = 120
    ...
    # movel规划
    def _movel_plan(self, pose: list, orientation: list, type: int, speed: int=10):
        msg = MoveL()
        msg.Pose.position.x = pose[0]
        msg.Pose.position.y = pose[1]
        msg.Pose.position.z = pose[2]
