...
class Robot_Action:
    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            # 循环遍历2个动作
            for index in range(self.action_times):
                ...
                for _ in range(4):
                    ...
                    if self._set_camera_status(True, frame_id):
                        ...
                        # 执行左臂抓取
                        self._secure_grip_arms(self.cargoes_name[index], frame_id, 1)
                        # 关闭相机
                        if self._set_camera_status(False, frame_id):
                            pass
                        else:
                    ...
                    if self._set_camera_status(True, frame_id):
                        ...
                        # 执行右臂识别抓取
                        self._secure_grip_arms(self.cargoes_name[index], frame_id, 2)
                        if self._set_camera_status(False, frame_id):
                            pass
                        else:
    ...
    # 双臂位资抓取+判断是否能够抓取成功
    def _secure_grip_arms(self, cargoes_name: str, frame_id: str, ctrl_type: int):
        # 将用到的变量写进字典
        ctrl_dict = {
            1: {
        ...
        if not self._wait_arm_planned(ctrl_type):
            rospy.logwarn("到达识别位置规划失败")
            return self._secure_grip_arms(cargoes_name, frame_id, ctrl_type)

        # 得到识别结果
        detect_result = self._get_material_detect_result(cargoes_name, frame_id, 20.0)
        ...
        if not self._wait_arm_planned(ctrl_type):
            rospy.logwarn("抓取动作1规划失败")
            return self._secure_grip_arms(cargoes_name, frame_id, ctrl_type)

        self._movejp_plan(q_, v_, ctrl_type)
        if not self._wait_arm_planned(ctrl_type):
            rospy.logwarn("抓取动作2规划失败")
            return self._secure_grip_arms(cargoes_name, frame_id, ctrl_type)
