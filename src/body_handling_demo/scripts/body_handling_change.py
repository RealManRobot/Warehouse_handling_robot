...
class Robot_Action:
    ...
    def _pick_up_the_box(self, detect_result):
        if detect_result.header.frame_id != '':
            ...
            # 规划到物体
            pose_vector = self._ar_tf_result(detect_result.pose, [0.138, -0.02, 0.083])
            self._movejp_plan(pose_vector[0]["orientation"], pose_vector[0]["position"], 1)
            self._movejp_plan(pose_vector[1]["orientation"], pose_vector[1]["position"], 2)
            # 等待双臂规划目标物完成
            self._wait_arm_planned(3)
    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            for index in range(len(self.cargo_boxes)):
                ...
                # 将双臂伸进
                self._double_movel_s(0, speed=20)
                # print(4)
                ...
                # 缩机械臂, 到达目标位
                self._double_movej_s(1, speed=20)
                ...
                # 双臂放置盒子
                self._double_movej_s(2, speed=20)
                ...
                # 双臂拿开
                self._double_movej_s(3, speed=20)
                ...
                # 双臂至识别目标物位姿
                self._double_movej_s(4, speed=20)
                ...
                # 缩机械臂
                self._double_movej_s(7, speed=20)
                ...
                # 将装好的框放置在货架上去伸机械臂 
                self._double_movej_s(8, speed=20)

                # 升降机至放置位
                self._set_lift_height(self.lift_height[self.cargo_boxes[index][1]] - 80, 60)
                self._wait_arm_planned(1)
                ...
                # 缩紧机械臂
                self._double_movel_s(1, speed=20)
