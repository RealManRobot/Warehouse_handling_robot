...
class Robot_Action:
    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            for index in range(self.cargo_box_num):
                ...
                # 线程抓取物料到物料箱子
                self.left_thread = threading.Thread(target=self._arm_actions_of_thread, args=("camera_left", 1, index, 20))
                self.right_thread = threading.Thread(target=self._arm_actions_of_thread, args=("camera_right", 2, index, 20))
    ...
    # 双臂抓取物料时的动作线程
    def _arm_actions_of_thread(self, frame_id: str, ctrl_type: int, cargoe_index: int, arm_speed: int):
        if ctrl_type == 1:
            arm_name = "左臂"
        else:
            arm_name = "右臂"
        ...
        while index < 4:
            ...
            # 过渡姿态
            self._movej_plan(self.ctrl_dict[ctrl_type]["joints"][1], ctrl_type)
            self._wait_arm_planned(ctrl_type)

            # 放置动作中容易发生碰撞，需要加上线程锁，避免两个臂同时放置物料到物料框中
            with self.placement_lock:
                # 放置姿态
                self._movej_plan(self.ctrl_dict[ctrl_type]["joints"][2], ctrl_type)
                self._wait_arm_planned(ctrl_type)

                self._grip_materiel("open", ctrl_type)
                rospy.sleep(3)
                # 识别姿态
                self._movej_plan(self.ctrl_dict[ctrl_type]["joints"][0], ctrl_type)
                self._wait_arm_planned(ctrl_type)
